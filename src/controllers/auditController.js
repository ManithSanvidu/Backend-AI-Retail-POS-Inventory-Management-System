      ...
      const events = [
        {
          ...
          severity: "HIGH",
          description:
            "Multiple failed login attempts detected from IP 203.0.113.45 - 15 attempts in 5 minutes",
        ...
        },
        {
          ...
          severity: "CRITICAL",
          description:
            "Attempted access to admin API endpoint without proper authorization token",
        ...
        },
        {
          ...
          severity: "MEDIUM",
          description:
            "Access from known malicious IP range - Multiple unusual activity patterns detected",
          ipAddress: "185.130.5.253",
          userName: "unknown",
          module: "SECURITY",
          metadata: {
            threatIntel: "Known malicious IP range",
            confidence: 85,
          },
          resolved: false,
        ...
        },
        {
          ...
          severity: "HIGH",
          description:
            "User attempted to escalate privileges via role modification API",
        ...
        },
        {
          ...
          severity: "MEDIUM",
          description:
            "Security policy was modified outside of maintenance window",
          ipAddress: "10.0.0.45",
          userName: "admin.user",
          module: "SECURITY",
          metadata: {
            changedFields: ["passwordPolicy", "lockoutPolicy"],
...
const getLoginAttempts = async (req, res) => {
  try {
    ...
    const getLocationFromIp = (ip) => {
      ...
      if (ip && ip.includes("192.168")) return "Local Network";
      return "Colombo, LK";
    };
    ...
    const formattedAttempts = attempts.map((a) => ({
      ...
      status: a.success
        ? "success"
        : a.failureReason === "ACCOUNT_LOCKED"
        ? "blocked"
        : "failed",
...
const blacklistIP = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `IP ${ipAddress} blacklisted.`,
      data: policy.ipPolicy,
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
const removeBlacklistIP = async (req, res) => {
  try {
    ...
    const policy = await SecurityService.removeFromBlacklist(
      ipAddress,
      req.user
    );
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc
        .fontSize(20)
        .font("Helvetica-Bold")
        .text("Audit & Security Report", { align: "center" });
      ...
      if (data.criticalEvents && data.criticalEvents.length > 0) {
        ...
        data.criticalEvents.slice(0, 10).forEach((event) => {
          ...
          doc.text(
            `• ${event.action} - ${event.module} - ${new Date(
              event.createdAt
            ).toLocaleDateString()}`
          );
        });
...
const generateExcelReport = async (data, res) => {
  try {
    ...
    summarySheet.addRow({
      ...
      value: data.summary?.failedLogins || 0,
    });

    summarySheet.addRow({
      metric: "Flagged Events",
      value: data.summary?.flaggedEvents || 0,
    });

    summarySheet.addRow({
      metric: "Critical Events",
      value: data.summary?.criticalEventsCount || 0,
    });
    ...
    if (data.breakdown?.byAction) {
      data.breakdown.byAction.forEach((item) =>
        actionsSheet.addRow({
          action: item._id,
          count: item.count,
        })
    ...
    if (data.criticalEvents) {
      data.criticalEvents.forEach((event) =>
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="audit-report-${Date.now()}.xlsx"`
    );
...
const downloadAuditReport = async (req, res) => {
  try {
    ...
    const reportData = await SecurityService.generateComplianceReport({
      startDate,
      endDate,
      branch,
    });
    ...
    if (format === "excel" || format === "xlsx") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="audit-report-${Date.now()}.csv"`
      );
